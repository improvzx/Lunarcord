...
$('#login-form').onsubmit=async e=>{e.preventDefault();try{const data=await api('/login',{method:'POST',body:JSON.stringify({email:$('#login-email').value,password:$('#login-password').value})});enterAccount(data);}catch(error){authMessage(error.message,true);}};
