...
function writeDb(db) { fs.mkdirSync(dataDir, { recursive: true }); fs.writeFileSync(dbFile, JSON.stringify(db, null, 2)); }
...
router.post('/register', async (req, res) => {
  ...
  const db = readDb(); if (db.users.some(u => u.email === email)) return res.status(409).json({ error: 'Este e-mail já está cadastrado.' });
  const code = String(Math.floor(100000 + Math.random() * 900000));
  db.users.push({ id: id(), email, username, passwordHash: await bcrypt.hash(password, 12), verified: false, verificationCode: code, codeExpires: Date.now() + 15 * 60_000, friends: [], requests: [] }); writeDb(db);
  const sent = await sendCode(email, code).catch(() => false);
  res.json({ ok: true, emailSent: sent, developmentCode: sent ? undefined : code });
});

router.post('/verify', (req, res) => {
  const email=String(req.body.email||'').toLowerCase(), code=String(req.body.code||''); const db=readDb(); const user=db.users.find(u=>u.email===email);
  ...
  user.verified=true; delete user.verificationCode; delete user.codeExpires; writeDb(db); res.json({ token: tokenFor(user), user: publicUser(user) });
});

router.post('/login', async (req, res) => {
  const db=readDb(), user=db.users.find(u=>u.email===String(req.body.email||'').toLowerCase());
...
router.get('/me', auth, (req,res)=>{ const db=readDb(), user=db.users.find(u=>u.id===req.userId); res.json({ user:publicUser(user), friends:user.friends.map(fid=>publicUser(db.users.find(u=>u.id===fid))).filter(Boolean), requests:user.requests.map(fid=>publicUser(db.users.find(u=>u.id===fid))).filter(Boolean), servers:db.servers.filter(s=>s.members.some(m=>m.userId===req.userId)) }); });
router.post('/friends/request', auth, (req,res)=>{ const db=readDb(), target=db.users.find(u=>u.email===String(req.body.email||'').toLowerCase()), me=db.users.find(u=>u.id===req.userId); if(!target||target.id===me.id)return res.status(404).json({error:'Usuário não encontrado.'}); if(!target.requests.includes(me.id)&&!target.friends.includes(me.id))target.requests.push(me.id); writeDb(db); res.json({ok:true}); });
router.post('/friends/accept', auth, (req,res)=>{ const db=readDb(), me=db.users.find(u=>u.id===req.userId), from=db.users.find(u=>u.id===req.body.userId); if(!from||!me.requests.includes(from.id))return res.status(404).json({error:'Pedido não encontrado.'}); me.requests=me.requests.filter(x=>x!==from.id); if(!me.friends.includes(from.id))me.friends.push(from.id); if(!from.friends.includes(me.id))from.friends.push(me.id); writeDb(db); res.json({ok:true}); });

router.post('/servers', auth, (req,res)=>{ const name=String(req.body.name||'').trim().slice(0,40); if(name.length<2)return res.status(400).json({error:'Digite um nome para o servidor.'}); const db=readDb(), server={id:id(),name,ownerId:req.userId,inviteCode:crypto.randomBytes(3).toString('hex').toUpperCase(),roles:[{id:'owner',name:'Dono',permissions:['ADMINISTRATOR']},{id:'member',name:'Membro',permissions:['VIEW_CHANNEL','SEND_MESSAGES','CONNECT','SPEAK','VIDEO','SHARE_SCREEN']}],members:[{userId:req.userId,roleId:'owner'}]}; db.servers.push(server);writeDb(db);res.json(server); });
router.post('/servers/join', auth, (req,res)=>{ const db=readDb(), server=db.servers.find(s=>s.inviteCode===String(req.body.inviteCode||'').toUpperCase());if(!server)return res.status(404).json({error:'Convite inválido.'});if(!server.members.some(m=>m.userId===req.userId))server.members.push({userId:req.userId,roleId:'member'});writeDb(db);res.json(server); });
router.get('/servers/:serverId', auth, (req,res)=>{const db=readDb(),server=db.servers.find(s=>s.id===req.params.serverId&&s.members.some(m=>m.userId===req.userId));if(!server)return res.status(404).json({error:'Servidor não encontrado.'});res.json({...server,members:server.members.map(m=>({...m,user:publicUser(db.users.find(u=>u.id===m.userId))}))});});
router.post('/servers/:serverId/roles', auth, (req,res)=>{ const db=readDb(), server=db.servers.find(s=>s.id===req.params.serverId);if(!server||server.ownerId!==req.userId)return res.status(403).json({error:'Somente o dono pode criar cargos.'});const role={id:id(),name:String(req.body.name||'Cargo').slice(0,30),permissions:Array.isArray(req.body.permissions)?req.body.permissions.filter(p=>['VIEW_CHANNEL','SEND_MESSAGES','CONNECT','SPEAK','VIDEO','SHARE_SCREEN','MANAGE_ROLES','KICK_MEMBERS'].includes(p)):[]};server.roles.push(role);writeDb(db);res.json(role); });
router.put('/servers/:serverId/members/:userId/role', auth, (req,res)=>{ const db=readDb(), server=db.servers.find(s=>s.id===req.params.serverId);if(!server||server.ownerId!==req.userId)return res.status(403).json({error:'Sem permissão.'});const member=server.members.find(m=>m.userId===req.params.userId);if(!member||!server.roles.some(r=>r.id===req.body.roleId))return res.status(404).json({error:'Membro ou cargo não encontrado.'});member.roleId=req.body.roleId;writeDb(db);res.json({ok:true}); });
